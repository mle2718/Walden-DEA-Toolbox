...
for(j in 1:J){
  (A[(M+1):(M+N),J+1]=-t(X[j,])) #column J+1 in A is being set to obs j input data
  (rhs=c(as.matrix(Y[j,]),rep(0,N),1)) #rhs is being set to obs j output data, 
                                       #0 for inputs and 1 fro VRS
  set.rhs(LP_API,rhs) 
...
#write.csv(objvals2, "~/John/joe/manual/vrs_input_04052019.csv")
#################################################################
# bootstrap using lpSolveAPI
#################################################################
set.seed(1001)
(n=J)
(m=round(sqrt(n),0))
...
for(b in 1:B){
  ...
  for(s in 1:J){
      ...
      boot[b,s]=round(get.objective(LP_B),3)
