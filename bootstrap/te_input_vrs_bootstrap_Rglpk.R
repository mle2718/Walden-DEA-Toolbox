...
status=0
for(j in (1:J)){
  A[(M+1):(M+N),(J+1)]=-A[(M+1):(M+N),j]         #replace last column in A for m outputs 
                                                 #by negative j A column for 
                                                 #output oriented model
  rhs=c(A[(1:M),j],rep(0,N),1)       #Zero for outpus, column j for inputs 
  #and one for VRS
  sol<- Rglpk_solve_LP(obj=obj, mat=A, dir=dir, rhs=rhs, max=F) #Solve using RGLPK
                                                                #max=F for minimization
  status[(j)]=sol$status
...
for(b in 1:B){
  ...
  for(s in 1:J){             
    (AB[,1]=A[,s])            #First column of Matrix AB is observation s From Matrix A
     AB[(M+1):(M+N),m+1]=-AB[(M+1):(M+N),1]    #for output oriented model. 
    #Last column outputs = negative first column outputs.
    
    rhsB=c((AB[(1:M),1]),rep(0,N),1)
    sol<- Rglpk_solve_LP(obj=objB, mat=AB, dir=dirB, rhs=rhsB, max=F) #Solve using RGLPK
...
#bias-corrected TE scores

round(mean(objvals2),3) #Calculates mean TE score for initial DEA Model
round(mean(S),3)        #Calculates mean bias-corrected TE score across all bootstrap runs
...
colnames(R)<-c("ITE","BCTE","ITE0.025","ITE0.975")
#####################################################################################
